...
pyseir run-all --run-mode=can-inference-derived --output-dir="${MODELS_DIR}" --state="Delaware"
...
mkdir -p dod_results
./run.py deploy-dod -is "${MODELS_DIR}/web_ui/state" -ic "${MODELS_DIR}/web_ui/county" -o "${RESULTS_DIR}"
