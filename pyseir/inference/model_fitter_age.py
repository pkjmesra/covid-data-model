...
def run_state(state, states_only=False):
    ...
    model_fitter_age = ModelFitterAge.run_for_fips(state_obj.fips)

    pd.DataFrame(model_fitter_age.fit_results, index=[state_obj.fips]).to_json(
    ...
    with open(os.path.join(state_output_dir, f'summary_{state}_state_only__mle_fit_results.pkl'), 'wb') as f:
        pickle.dump(model_fitter_age.mle_model, f)
    ...
    if not states_only:
        ...
        p = Pool()
        fitters = p.map(ModelFitterAge.run_for_fips, all_fips)
        p.close()
